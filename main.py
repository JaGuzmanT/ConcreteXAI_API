...
def st_sidebar_images(image_1, image_2, image_3):
    st.sidebar.image(image=image_1, caption="IngenierIA Soluciones", width=150)
    st.sidebar.image(image=image_2, caption="UMSNH", width=150)
    st.sidebar.image(image=image_3, caption="Facultad de Ingeniería Civil", width=150)           
...
Er = st.number_input(label="Electrical Resistivity (Ω-cm):", value=None, placeholder="Type a number", min_value=0.00, max_value=20.00, step=1)
Ultras = st.number_input(label="Ultrasonic Pulse Velicty (m/s):", value=None, placeholder="Type a number", min_value=0.00, max_value=6000.00, step=1)
st.divider()
#######################################################################################
